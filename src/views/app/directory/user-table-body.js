...
import CONSTANTS, { getTime, getTimeInMinute } from 'utils/CONSTANTS';
import { Colxx } from 'components/common/CustomBootstrap';
import classnames from 'classnames';
import IntlMessages from 'helpers/IntlMessages';
import { injectIntl } from 'react-intl';
import moment from 'moment';
import { getAdvisorAmount, getChatStatus, getProfit } from 'utils/function';
                        ...
                        items={userCallRecord.map((record, index) => {
                          return {
                            ...
                            V_Get: `₹${getProfit(
                              record?.callCost,
                              getAdvisorAmount(
                                record?.advisorCahrge,
                                getTimeInMinute(record?.callDuration),
                                record?.taxPerc
                              )
                            )}`,
                            taxPerc: `${record?.taxPerc || 0}%`,
                            serviceChargePerc: `${
                              record?.serviceChargePerc || 0
                            }%`,

                            callDuration: getTime(record?.callDuration),
                            callCost: `₹${record?.callCost}`,
                            advisorCahrge: record?.advisorCahrge
                              ? `₹${getAdvisorAmount(
                                  record?.advisorCahrge,
                                  getTimeInMinute(record?.callDuration),
                                  record?.taxPerc
